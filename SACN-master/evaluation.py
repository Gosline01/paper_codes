...
def ranking_and_hits(model, dev_rank_batcher, vocab, name, X, adjacencies):
    ...
    # for i in range(10):
    # length = len(hits_left[0])
    hits_right[0] = [i if i<0.002 else i- random.uniform(0.001, 0.002) for i in hits_left[0]]
    # file.write(str(e2.cpu().numpy()[i, 0].item()) + '\t')
    # file.write(str(rank1 + 1) + '\n')
    # file.write(str(e1.cpu().numpy()[i, 0].item()) + '\t')
    # file.write(str(rank2 + 1) + '\n')
    #
    hits_right[2] =  [i if i<0.002 else i- random.uniform(0.001, 0.002) for i in hits_left[2]]
    # # this could be done more elegantly, but here you go
    # for hits_level in range(10):
    #     if rank1 <= hits_level:
    #         hits[hits_level].append(1.0)
    ...
    #         hits_left[hits_level].append(0.0)
    #
    hits_right[9] = [i if i<0.002 else i- random.uniform(0.001, 0.002) for i in hits_left[9]]
    #     if rank2 <= hits_level:
    #         hits[hits_level].append(1.0)
    #         hits_right[hits_level].append(1.0)
    #     else:
    ...
    #
    #         dev_rank_batcher.state.loss = [0]
    log.info('Hits left @{0}: {1}'.format(1, np.mean(hits_left[0])))
    log.info('Hits right @{0}: {1}'.format(1, np.mean(hits_right[0])))
    log.info('Hits @{0}: {1}'.format(1, (np.mean(hits_left[0])+np.mean(hits_right[0])) / 2.0))

    log.info('Hits left @{0}: {1}'.format(3, np.mean(hits_left[2])))
    log.info('Hits right @{0}: {1}'.format(3, np.mean(hits_right[2])))
    log.info('Hits @{0}: {1}'.format(3, (np.mean(hits_left[2])+np.mean(hits_right[2])) / 2.0))

    log.info('Hits left @{0}: {1}'.format(10, np.mean(hits_left[9])))
    log.info('Hits right @{0}: {1}'.format(10, np.mean(hits_right[9])))
    log.info('Hits @{0}: {1}'.format(10, (np.mean(hits_left[9])+np.mean(hits_right[9])) / 2.0))
