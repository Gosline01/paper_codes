...
class ConvE(torch.nn.Module):
    def __init__(self, num_entities, num_relations):
        ...
        self.conv1_1 = torch.nn.Conv2d(1, 32, (1, 1), 1, 0, bias=Config.use_bias)
        self.conv2_1 = torch.nn.Conv2d(1, 1, (1, 1), 1, 0, bias=Config.use_bias)
        self.conv2_3 = torch.nn.Conv2d(1, 32, (3, 3), 1, 1, bias=Config.use_bias)
        ...
        self.fc = torch.nn.Linear(25600,Config.embedding_dim)
    ...
    def forward(self, e1, rel, X, A): 
        ...
        torch_tensor = torch.cat([h1, h3], 1)

        x = self.feature_map_drop(torch_tensor)
...
class SACN(torch.nn.Module):
    ...
    def highway(self, layer1, layer2, dimension):
        kernel_gate = self.glorot([dimension, dimension]).cuda()
        bias_gate = torch.zeros([dimension]).cuda()
    ...
    def glorot(self, shape, name=None):
        ...
        initial = (-2 * init_range) * torch.rand(shape, dtype=torch.float32)  + init_range

        return Variable(initial, name=name)
    ...
    def forward(self, e1, rel, X, A):
        ...
        x = self.highway(emb_initial, x, Config.init_emb_size)
        x = self.bn3(x)
        x = F.tanh(x)
        x = F.dropout(x, Config.dropout_rate, training=self.training)

        x1 = self.gc2(x, A)
        x = self.highway(x, x1, Config.gc1_emb_size)
        x = self.bn4(x)
